...
export default function Agent12() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 12;
  const Icon = icons[12 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Leo</h3>
        <p className="text-xs text-slate-400">Router Manager</p>
      </div>
      {isActive && (
