...
export default function Agent14() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 14;
  const Icon = icons[14 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Ned</h3>
        <p className="text-xs text-slate-400">State Manager</p>
      </div>
      {isActive && (
