...
export default function Agent15() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 15;
  const Icon = icons[15 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Oli</h3>
        <p className="text-xs text-slate-400">Database Admin</p>
      </div>
      {isActive && (
