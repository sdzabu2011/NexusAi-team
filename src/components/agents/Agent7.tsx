...
export default function Agent7() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 7;
  const Icon = icons[7 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Gia</h3>
        <p className="text-xs text-slate-400">Cloud Specialist</p>
      </div>
      {isActive && (
