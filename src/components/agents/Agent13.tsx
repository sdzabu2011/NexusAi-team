...
export default function Agent13() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 13;
  const Icon = icons[13 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Moe</h3>
        <p className="text-xs text-slate-400">Performance Eng</p>
      </div>
      {isActive && (
