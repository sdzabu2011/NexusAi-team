...
export default function Agent10() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 10;
  const Icon = icons[10 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Jax</h3>
        <p className="text-xs text-slate-400">Bug Finder</p>
      </div>
      {isActive && (
