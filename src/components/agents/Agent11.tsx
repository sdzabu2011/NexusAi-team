...
export default function Agent11() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 11;
  const Icon = icons[11 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Kai</h3>
        <p className="text-xs text-slate-400">Visual Inspector</p>
      </div>
      {isActive && (
