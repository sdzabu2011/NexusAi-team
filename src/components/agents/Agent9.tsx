...
export default function Agent9() {
  const { isGenerating, activeAgentId } = useAppStore();
  const isActive = activeAgentId === 9;
  const Icon = icons[9 as keyof typeof icons] || Code;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-center">
        <h3 className="text-sm font-bold text-white">Ian</h3>
        <p className="text-xs text-slate-400">QA Tester</p>
      </div>
      {isActive && (
